...
for(i in 1:N_iter){
  ...
  results_int$log_age_log_base_treat_int_b[i] = coef(temp_m)[7]
...
```{R}
#| label: tbl-coef-comp
#| fig-cap: "Comparison of coefficients and standard errors from bootrapp iterations" 
#| 
###################
    ...
    summarize(
      across(
        .cols = c('log_age_b', 'log_base_b', 'treat_b', 
                  'log_age_log_base_int_b', 'log_age_treat_int_b', 'log_age_log_base_treat_int_b'), 
        .fns = list(mean, sd)
    )) 

mean_int <- int_model_coef[c(1,3,5,7,9,11)]
var_names <- names(mean_int)
names(mean_int) <- NULL

sd_int <- int_model_coef[c(2,4,6,8,10,12)]
names(sd_int) <- NULL
...
coef_from_two_model <- 
  ...
  mutate(
    pretty_names = c("Log(Age)", "Log(Baseline Seizures)", "Treatment", "Log(Age):Log(Baseline Seizures)", 
                     "Log(Age):Treatment", "Log(Age):Log(Baseline Seizures):Treatment")
  ) %>% 
  
  select(pretty_names, everything(), -names) 
...
coef_from_two_model %>% 
  ...
  kable(booktabs = T, 
        ...
        digits = 2, 
        col.names = c("Model Terms", "Beta from Model w/ Interactions", "SE of Beta from Model w/ Interactions",
                      "Beta from Simple Model", "SE of Beta from Simple Model")) %>% 
  kable_styling(latex_options = c("condenced", "striped", "hold_position"), 
                full_width = F) %>% 
  column_spec(1:length(coef_from_two_model), width = "3cm")
```
